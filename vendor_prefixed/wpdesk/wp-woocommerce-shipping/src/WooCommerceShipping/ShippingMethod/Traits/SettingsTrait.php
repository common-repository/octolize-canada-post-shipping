...
use OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\ShippingMethod\RulesTableAdv;
/**
    ...
    private function replace_fallback_field_if_exists($settings)
    {
        ...
        foreach ($settings as $key => $field) {
            if ($field['type'] === FallbackRateMethod::FIELD_TYPE_FALLBACK) {
                $new_settings[FallbackRateMethod::FIELD_ENABLE_FALLBACK] = ['title' => __('Fallback', 'octolize-canada-post-shipping'), 'type' => 'checkbox', 'label' => __('Enable fallback', 'octolize-canada-post-shipping'), 'description' => __('Enable to offer flat rate cost for shipping so that the user can still checkout, if API for some reason returns no matching rates.', 'octolize-canada-post-shipping'), 'desc_tip' => \true, 'default' => 'no'];
                $new_settings[FallbackRateMethod::FIELD_FALLBACK_COST] = ['title' => __('Fallback Cost', 'octolize-canada-post-shipping'), 'type' => 'price', 'required' => \true, 'description' => __('Enter only a numeric value without the currency symbol.', 'octolize-canada-post-shipping'), 'desc_tip' => \true, 'default' => ''];
            } else {
