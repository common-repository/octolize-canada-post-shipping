    ...
    private function add_rates(CanRate $service, ShippingMethod $method, WooCommerceShippingMetaDataBuilder $meta_data_builder, WooCommerceShippingBuilder $shipping_builder)
    {
        ...
        if ($service->is_rate_enabled($service_settings)) {
            ...
            if ($method instanceof HasRateCache) {
                $cached_rating = new CachedRating((new \OctolizeShippingCanadaPostVendor\WPDesk\WooCommerceShipping\Cache\ShopSettingsMd5HashGenerator())->generate_md5_hash(new ShopSettings($service_id)), WC()->session ? new WooCommerceSessionContainer(WC()->session) : new ArrayContainer());
