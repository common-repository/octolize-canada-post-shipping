...
namespace OctolizeShippingCanadaPostVendor\Octolize\Onboarding\PluginUpgrade;

use OctolizeShippingCanadaPostVendor\Octolize\Onboarding\Field\Html;
use OctolizeShippingCanadaPostVendor\Octolize\Onboarding\Onboarding;
use OctolizeShippingCanadaPostVendor\Octolize\Onboarding\OnboardingButton;
use OctolizeShippingCanadaPostVendor\Octolize\Onboarding\OnboardingOption;
use OctolizeShippingCanadaPostVendor\Octolize\Onboarding\OnboardingShouldShowAlwaysStrategy;
use OctolizeShippingCanadaPostVendor\Octolize\Onboarding\OnboardingShouldShowStrategy;
use OctolizeShippingCanadaPostVendor\Octolize\Onboarding\OnboardingStep;
use OctolizeShippingCanadaPostVendor\Octolize\Onboarding\OnboardingTrackerData;
...
class PluginUpgradeOnboardingFactory
{
    ...
    private function prepare_steps(string $previous_version, string $current_version): array
    {
        ...
        $onboarding_step->set_show(\true)->set_heading(sprintf(__('Thank you for updating %1$s!', 'octolize-canada-post-shipping'), $this->plugin_name))->set_sub_heading(__('It is really important to keep the plugins up to date. We have implemented some improvements and new functionalities. Find out what has changed:', 'octolize-canada-post-shipping'));
    ...
    {
        return [(new OnboardingButton())->set_label(__('I\'m not interested', 'octolize-canada-post-shipping'))->set_classes(OnboardingButton::BTN_LINK), (new OnboardingButton())->set_label(__('Thanks for letting me know', 'octolize-canada-post-shipping'))->set_type(OnboardingButton::TYPE_CLOSE)];
    }
